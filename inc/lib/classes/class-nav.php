...
class Mobile_Walker_Nav_Menu extends Walker_Nav_Menu {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		$item_output .= '<a'. $attributes .'>';
		$item_output .= $args->link_before . $title . $args->link_after;
		$item_output .= '</a>'.(in_array( 'menu-item-has-children', $item->classes ) ? '<button type="button" class="sub-nav-toggle" title="open sub-navigation"><i class="material-icons">arrow_forward</i></button>' : '');
		$item_output .= $args->after;
		/**
		 * Filter a menu item's starting output.
		 *
    ...
    $output .= "\n" . $indent . '<ul class="' . $class_names . '"><button type="button" class="sub-nav-close" title="close sub-navigation"><i class="material-icons">close</i></button>' . "\n";
  }
}
